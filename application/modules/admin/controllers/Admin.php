...
class Admin extends MY_Controller
{
    ...
    public function settings($type = 'r')
    {
        ...
        if ($type == 'r') {
            ...
            $this->setting_model->update([
                'key' => 'delivery_range',
                'value' => $this->input->post('delivery_range'),
            ],'key');
            $this->setting_model->update([
                'key' => 'vendor_range',
                'value' => $this->input->post('vendor_range'),
            ],'key');
            redirect('settings/r', 'refresh');
        }elseif ($type == 'appsettigs'){
            $this->setting_model->update([
                'key' => 'otplogin',
                'value' => $this->input->post('otplogin'),
            ],'key');
            $this->setting_model->update([
                'key' => 'google_api_key',
                'value' => $this->input->post('google_api_key'),
            ],'key');
            $this->setting_model->update([
                'key' => 'app_notification_status',
                'value' => $this->input->post('app_notification_status'),
            ],'key');
            $this->setting_model->update([
                'key' => 'app_notification',
                'value' => $this->input->post('app_notification'),
            ],'key');
            redirect('settings/r', 'refresh');
        }elseif ($type == 'news'){
            $this->setting_model->update([
