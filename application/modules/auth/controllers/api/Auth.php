...
class Auth extends MY_REST_Controller
{
    ...
    public function userlogin_post()
    {
        ...
        if(!empty($mobile)){
            ...
            if(!empty($otp) || ($this->setting_model->where('key','otplogin')->get()['value'] == 0)){
                if((!empty($userotp) && ($otp == $userotp)) || ($this->setting_model->where('key','otplogin')->get()['value'] == 0)){
                      $is_user_exist = $this->user_model->where('phone', $mobile)->get();
            if(empty($is_user_exist)){
                ...
                if(!empty($group)){
                    ...
                    $email='';
                    $identity = ($this->config->item('identity', 'ion_auth') === 'email') ? $mobile : $unique_id;
                ...
                //$login = $this->user_model->where('unique_id', $is_auth_id_exist['unique_id'])->get();
                    ...
                    $this->set_response_simple([
                        "message" => 'Invalid OTP',
                    ], 'OTP Generated.!', http_response_code(), TRUE);
                }
            }else{
       ...
       /* $this->form_validation->set_rules($this->user_model->rules['userlogin']);
